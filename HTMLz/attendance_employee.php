...
if (
    ...
    if (mysqli_num_rows($resultEmployee) > 0) {
        ...
        if ($currentDateTime >= $morningStart && $currentDateTime <= $morningEnd) {
            ...
            if ($action === 'Time In') {
                $status = 'Time In';
                $sqlUpdate = "INSERT INTO attendance (name, afternoon_time_in, status) VALUES ('$employeeName', '$currentDateTime', '$status') ON DUPLICATE KEY UPDATE afternoon_time_in='$currentDateTime', status='$status'";
                ...
                $status = 'Time Out';
                $sqlUpdate = "UPDATE attendance SET afternoon_time_out = '$currentDateTime', afternoon_total_hours = TIMESTAMPDIFF(SECOND, afternoon_time_in, '$currentDateTime'), status = '$status' WHERE name = '$employeeName' AND afternoon_time_out IS NULL";
            ...
            if ($action === 'Overtime Time In') {
                ...
                $sqlUpdate = "INSERT INTO attendance (name, overtime_time_in, status) VALUES ('$employeeName', '$currentDateTime', '$status') 
                ON DUPLICATE KEY UPDATE overtime_time_in='$currentDateTime', status='$status'";
